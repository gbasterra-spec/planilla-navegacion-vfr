...
# NUEVA CONFIGURACIÓN: FORMATO VERTICAL A4 (PORTRAIT)
def generar_pdf_a4_vertical(tabla_datos, dist_t, tiempo_t, comb_t, avion, var, v_dir, v_vel):
    buffer = BytesIO()
    
    # Tamaño A4 vertical (portrait) con márgenes de 30pt (ancho imprimible ~535pt)
    doc = SimpleDocTemplate(
        buffer, 
        pagesize=pdf_sizes.A4, 
        rightMargin=30, leftMargin=30, 
        topMargin=30, bottomMargin=30
    )
    
    styles = getSampleStyleSheet()
    title_style = ParagraphStyle('PdfTitle', parent=styles['Heading1'], fontSize=13, leading=16, textColor=colors.HexColor('#1E3A8A'), alignment=1, spaceAfter=8)
    normal_style = ParagraphStyle('PdfNormal', parent=styles['Normal'], fontSize=8.5, leading=11, textColor=colors.black)
    header_style = ParagraphStyle('PdfHeader', parent=styles['Normal'], fontSize=8, leading=10, textColor=colors.white, fontName='Helvetica-Bold', alignment=1)
    cell_style = ParagraphStyle('PdfCell', parent=styles['Normal'], fontSize=8, leading=10, alignment=1)
    frec_style = ParagraphStyle('PdfFrec', parent=styles['Normal'], fontSize=8, leading=11, textColor=colors.HexColor('#334155'))
    
    story = []
    story.append(Paragraph("<b>PLANILLA DE NAVEGACIÓN VFR</b>", title_style))
    ...
    total_style = ParagraphStyle('PdfTotal', parent=styles['Normal'], fontSize=8, fontName='Helvetica-Bold', alignment=1)
    pdf_rows.append([
    ...
    # Anchos de columna optimizados para el ancho disponible de A4 vertical (535pt)
    tabla_tramos = Table(pdf_rows, colWidths=[38, 115, 45, 34, 34, 34, 38, 36, 45, 42, 74])
    ...
    # BLOQUE INFERIOR DE FRECUENCIAS DE RADIO
    ...
    tabla_frec = Table(frec_data, colWidths=[535])
...
st.set_page_config(page_title="Planilla de Navegación A4", page_icon="📋", layout="wide")

st.title("📋 Planilla de Navegación Operativa (A4 Vertical)")
st.write("Planilla de vuelo en formato vertical A4 con previsualización de ruta en mapa.")
...
if not lista_oaci:
    ...
    if total_distancia > 0:
        ...
        pdf_data = generar_pdf_a4_vertical(
            tabla_navegacion, total_distancia, total_tiempo, total_combustible, 
            avión_sel, var_mag, dir_viento, vel_viento
        )
        
        st.download_button(
            label="📥 Descargar Planilla de Navegación en PDF (A4 Vertical)",
            data=pdf_data,
            file_name="Planilla_VFR_A4_Vertical.pdf",
